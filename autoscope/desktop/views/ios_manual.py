"""iOS Manual recorder view for the desktop app."""
...
class IOSManualViewMixin:
    def _show_ios_manual(self) -> None:
        ...
        def start_stream(_: ft.ControlEvent) -> None:
            if self._ios_recorder and self._ios_recorder.is_recording:
                _snack(self.page, "Already streaming")
                return
            try:
                self._ios_recorder = IOSRecorder(self.config)
                self._ios_recorder.set_callbacks(add_action_log, on_frame)
                self._ios_recorder.set_status_callback(on_status)
                status_text.value = "Connecting to WebDriverAgent..."
                status_text.update()
                ...
                def launch() -> None:
                    try:
                        self._ios_recorder.start(name_field.value, record_video=video_checkbox.value)
                        _ui_call(
                            self.page,
                            lambda: _snack(
                                self.page,
                                f"Connected to {self._ios_recorder.wda_url}",
                            ),
                        )
                        _ui_call(
                            self.page,
        ...
        def stop_stream(_: ft.ControlEvent) -> None:
            if not self._ios_recorder or not self._ios_recorder.is_recording:
            ...
            def stop() -> None:
                try:
                    path = self._ios_recorder.stop()
                    video_path = self._ios_recorder.video_path
        ...
        def take_screenshot(_: ft.ControlEvent) -> None:
            ...
            self._ios_recorder.take_screenshot(name)
        ...
        def send_text(_: ft.ControlEvent) -> None:
            ...
            if not text:
                return
            self._ios_recorder.input_text(text)
        ...
        def on_tap(e: ft.TapEvent) -> None:
            ...
            self._ios_recorder.tap(x, y, container_width, container_height)
        ...
        def on_pan_end(_: ft.DragEndEvent) -> None:
            ...
            self._ios_recorder.swipe(x1, y1, x2, y2, container_width, container_height)
