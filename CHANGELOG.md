...
### Added
- **iOS support** (first milestone, Simulator-focused): a new `IOSDriver` (`autoscope/drivers/ios.py`) connects to an already-running WebDriverAgent instance via the `wda` client — `self.ios` in `AutomateTestCase` (tag `"ios"` / `needs_ios = True`) is a `wda.Client` session supporting `.tap()`, `.swipe()`, `.screenshot()`, element queries, etc. New `IOSConfig` (`wda_url`, `bundle_id`, `screenshot_dir`, `video_dir`), `tests_ios/` example tests, and `tools/start_ios_wda.sh` to build/launch WebDriverAgent against a booted Simulator (clones into `~/.autoscope/WebDriverAgent` on first run). Scoped to CLI/`AutomateTestCase` only for now — no desktop recorder UI, no video recording, and macOS/Xcode only (an Apple tooling requirement, not ours). Verified against a real Simulator: screenshot, tap (launched Safari), app launch/terminate by bundle ID, and text input all confirmed working end-to-end through the actual test harness.
